...
class SessionController extends ApiController {
    ...
    public function checkSessionIframe(): DataDisplayResponse {
        ...
        $html = '<!doctype html><html><head><meta charset="utf-8"><title>OIDC Session Management</title></head><body><script>'
            ...
            . 'function b64uDecode(v){try{if(typeof v!=="string"||!/^[A-Za-z0-9_-]+$/.test(v)||v.length%4===1){return null;}var original=v,b64=v.replace(/-/g,"+").replace(/_/g,"/");while(b64.length%4){b64+="=";}var decoded=atob(b64),canonical=btoa(decoded).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"");return canonical===original?decoded:null;}catch(x){return null;}}'
