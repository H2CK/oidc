...
- Token Exchange is an **administrative delegation authorization**. A configured allow-list relationship intentionally overrides the normal per-user consent requirement for the requesting client; Token Exchange does not create or require a separate user-consent record for that client. Administrators must therefore grant these relationships only between clients that are trusted for delegated access.
...
- Resource-bound access tokens are accepted by the UserInfo endpoint only when their resource exactly equals the provider's discovered `userinfo_endpoint`. A TEX token targeted at another backend resource is rejected by UserInfo with HTTP 401 / `invalid_token`. Unbound access tokens keep the existing UserInfo behavior. To intentionally issue a TEX token for UserInfo, configure the exact discovered `userinfo_endpoint` as a Token Exchange target and request that URI as `resource`.
...
In the Admin UI, **Allowed Token Exchange Subject Clients** is a multi-select list of client IDs. At least one client must be selected before Token Exchange can be enabled. Select the requesting client itself if same-client exchange is required; select additional client IDs only for explicitly trusted cross-client delegation paths.
...
When a newly created client should accept its own tokens, an explicit `--client_id` is required so the same identifier can also be supplied via `--tex_allowed_subject_client`. Existing clients that had Token Exchange enabled before the subject-client allow-list was introduced receive **no implicit trust entries during migration** and therefore fail closed until an administrator configures at least one allowed subject client.
